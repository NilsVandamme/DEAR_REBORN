...
public class SC_PullOfWord : MonoBehaviour
{
    ...
    private void WriteWordAndCL()
    {
        ...
        foreach (SC_CLInPull cl in SC_GM_Master.gm.wordsInPull)
            for (int i = 0; i < champsLexicauxAndWords.Length; i++)
                if (cl.GetCL() == champsLexicauxAndWords[i][posElemCl].text)
                    foreach (SC_Word word in cl.GetListWord())
                        champsLexicauxAndWords[i][GetFirstMotLibre(i)].text = word.titre;
    ...
    */
    ...
    //    foreach (SC_CLInPull cl in SC_GM_Master.gm.wordsInPull)
    //        if (cl.GetCL() == champsLexicauxAndWords[index][posElemCl].text)
    //        {
    //            foreach (Word word in cl.GetListWord())
    //            {
    ...
    //                        champsLexicauxAndWords[index][pos].text = word.titre;
    //                    }
    ...
    //                        {
    //                            hasWord[index][pos] = true;
    //                            champsLexicauxAndWords[index][pos].text = word.grammarCritere[item.Value];
    //                        }
    //                }
    //            }
    //        }
    ...
    //                    {
    //                        SC_CLInPull mot = GetWordInPull(champsLexicauxAndWords[k][l].text);
     ...
     */
    private SC_Word GetWordInPull(string mot)
    {
        foreach (SC_CLInPull cl in SC_GM_Master.gm.wordsInPull)
            foreach (SC_Word word in cl.GetListWord())
            {
                ...
                foreach (KeyValuePair<string, int> item in critereOfWord)
                    if (mot == word.grammarCritere[item.Value])
                        return word;
            }
    ...
    public void AddWordInWheel(TextMeshProUGUI tmp)
    {
        ...
        for (int i = 0; i < champsLexicauxAndWords.Length; i++)
            for (int j = 0; j < champsLexicauxAndWords[i].Length; j++)
                ...
                {
                    SC_Word word = GetWordInPull(champsLexicauxAndWords[i][j].text);
                    if (word != null)
                    {
     ...
     public void OnClickRemoveWordInWheel(TextMeshProUGUI tmp)
    {
        for (int i = 0; i < listOfWheel.Length; i++)
            if (listOfWheel[i] == tmp)
            {
                ...
                SC_GM_Local.gm.wheelOfWords.Remove(GetWordInPull(listOfWheel[i].text));
                listOfWheel[i].text = "";
            }
